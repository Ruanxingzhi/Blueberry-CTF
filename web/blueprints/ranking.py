...
WITH t AS (
  SELECT user_id, SUM(point) AS total_point, MAX(submit_time) AS last_submit
  FROM view_user_solve JOIN problem ON problem.id = view_user_solve.problem_id
  WHERE problem.is_visible
  GROUP BY user_id
)
